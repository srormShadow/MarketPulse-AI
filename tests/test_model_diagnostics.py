...
def test_analyze_category_model_returns_correct_structure(db_session):
    """Test that analyze_category_model returns expected structure."""
    _seed_diagnostic_data(db_session)

    result = analyze_category_model(db_session, "DiagCategory")
...
def test_analyze_category_model_has_all_features(db_session):
...
def test_analyze_category_model_coefficients_are_numeric(db_session):
...
def test_analyze_category_model_importance_is_absolute(db_session):
...
def test_analyze_category_model_insufficient_data(db_session):
    ...
    for dt in dates:
        session.add(Sales(date=dt.date(), sku_id="SPARSE", units_sold=100))

    session.commit()

    with pytest.raises(ValueError, match="Insufficient data"):
        analyze_category_model(session, "SparseCategory")


def test_compare_categories_returns_dataframe(db_session):
    """Test that compare_categories returns a DataFrame."""
    _seed_diagnostic_data(db_session)

    result = compare_categories(db_session, ["DiagCategory", "DiagCategory2"])
...
def test_compare_categories_has_all_features(db_session):
...
def test_compare_categories_skips_invalid(db_session):
    """Test that compare_categories skips categories with insufficient data."""
    _seed_diagnostic_data(db_session)

    # Should not raise error, just skip invalid category
    result = compare_categories(db_session, ["DiagCategory", "NonExistent"])

    assert len(result) == 1
    assert "DiagCategory" in result.index


def test_compare_categories_empty_raises_error(db_session):
    """Test that no valid categories raises ValueError."""
    with pytest.raises(ValueError, match="No categories had sufficient data"):
        compare_categories(db_session, ["NonExistent1", "NonExistent2"])


def test_rank_feature_importance_returns_dataframe(db_session):
    """Test that rank_feature_importance returns DataFrame."""
    _seed_diagnostic_data(db_session)

    result = rank_feature_importance(db_session, ["DiagCategory", "DiagCategory2"], "festival_score")
...
def test_rank_feature_importance_sorted_by_magnitude(db_session):
    """Test that ranking is sorted by absolute coefficient magnitude."""
    _seed_diagnostic_data(db_session)

    result = rank_feature_importance(db_session, ["DiagCategory", "DiagCategory2"], "lag_1")
...
def test_rank_feature_importance_invalid_feature(db_session):
    """Test that invalid feature name returns zero coefficient."""
    _seed_diagnostic_data(db_session)

    result = rank_feature_importance(db_session, ["DiagCategory"], "invalid_feature")
...
def test_summarize_category_behavior_returns_dict(db_session):
    """Test that summarize_category_behavior returns dict."""
    _seed_diagnostic_data(db_session)

    result = summarize_category_behavior(db_session, "DiagCategory")
...
def test_summarize_category_behavior_festival_classification(db_session):
...
def test_summarize_category_behavior_momentum_boolean(db_session):
...
def test_summarize_category_behavior_stability_classification(db_session):
...
def test_summarize_category_behavior_summary_is_string(db_session):
...
def test_compare_feature_sensitivity_returns_dict(db_session):
    """Test that compare_feature_sensitivity returns dict."""
    _seed_diagnostic_data(db_session)

    result = compare_feature_sensitivity(db_session, ["DiagCategory", "DiagCategory2"])

    assert isinstance(result, dict)


def test_compare_feature_sensitivity_has_expected_keys(db_session):
...
def test_compare_feature_sensitivity_structure(db_session):
...
def test_compare_feature_sensitivity_identifies_leader(db_session):
...
def test_coefficients_differ_across_categories(db_session):
    """Test that different categories learn different coefficients."""
    _seed_diagnostic_data(db_session)

    comparison = compare_categories(db_session, ["DiagCategory", "DiagCategory2"])
...
def test_dominant_feature_identified(db_session):
...
def test_n_samples_reasonable(db_session):
...
def test_intercept_is_reasonable(db_session):
