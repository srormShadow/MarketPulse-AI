...
class DataRepository(Protocol):
    ...
    def count_shopify_stores(self) -> int: ...
...
class SQLiteRepository:
    ...
    def count_skus(self) -> int:
        stmt = select(func.count()).select_from(SKU)
    ...
    def count_shopify_stores(self) -> int:
        stmt = select(func.count()).select_from(ShopifyStore).where(ShopifyStore.is_active == True)
        return self._db.scalar(stmt) or 0
