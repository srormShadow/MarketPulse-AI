"""Festival calendar and prediction routes."""
...
def list_festivals(
    ...
    Per-category rows are merged into a single entry per festival so the
    ...
    for row in rows:
        ...
        entry["max_uplift"] = max(entry["max_uplift"], uplift)

    items: list[FestivalItemResponse] = []
    for entry in grouped.values():
        max_uplift = entry["max_uplift"]
        ...
        items.append(
            FestivalItemResponse(
                ...
                historical_uplift=max_uplift,
                demand_multiplier=1.0 + max_uplift,
                days_until=days_until,
                category_uplifts=entry["category_uplifts"],
            )
        )
